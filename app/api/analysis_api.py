...
@api_error_handler(default_message='股票筛选失败')
def screen_stocks():
    """股票筛选"""
...
def backtest_strategy():
    ...
    try:
        ...
        message = str(e) if is_debug else '参数错误，请检查输入'
        return jsonify({
            'code': 400,
            'message': message,
            'data': None
        ...
        from config import Config
        is_debug = getattr(Config, 'DEBUG', False)
        logger.error(f"策略回测API错误: {e}", exc_info=True)
        message = f'回测失败: {str(e)}' if is_debug else '回测执行失败，请稍后重试'
        return jsonify({
            'code': 500,
            'message': message,
            'data': None
...
class BacktestEngine:
    ...
    def _bollinger_strategy(self, df):
        """布林带策略（修复：检查前一日状态避免重复触发信号）"""
        prev_touch_upper = False   # 前一天是否触及上轨
        prev_touch_lower = False   # 前一天是否触及下轨
        ...
        for i in range(len(df)):
            ...
            touch_lower = close <= boll_lower and boll_lower > 0
            touch_upper = close >= boll_upper and boll_upper > 0

            # 价格从上方回落到触及下轨（之前没在下轨 → 现在到了下轨）：买入信号
            if touch_lower and not prev_touch_lower:
                df.iloc[i, df.columns.get_loc('signal')] = 1
            # 价格从下方回升到触及上轨（之前没在上轨 → 现在到了上轨）：卖出信号
            elif touch_upper and not prev_touch_upper:
                df.iloc[i, df.columns.get_loc('signal')] = -1

            prev_touch_lower = touch_lower
            prev_touch_upper = touch_upper
