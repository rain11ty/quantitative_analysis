...
class StockBusiness(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            ...
            # 基本数据
            'daily_close': float(self.daily_close) if self.daily_close is not None else None,
            ...
            # 技术因子
            'factor_open': float(self.factor_open) if self.factor_open is not None else None,
            'factor_high': float(self.factor_high) if self.factor_high is not None else None,
            'factor_low': float(self.factor_low) if self.factor_low is not None else None,
            'factor_pre_close': float(self.factor_pre_close) if self.factor_pre_close is not None else None,
            'factor_change': float(self.factor_change) if self.factor_change is not None else None,
            'factor_pct_change': float(self.factor_pct_change) if self.factor_pct_change is not None else None,
            'factor_vol': float(self.factor_vol) if self.factor_vol is not None else None,
            'factor_amount': float(self.factor_amount) if self.factor_amount is not None else None,
            'factor_adj_factor': float(self.factor_adj_factor) if self.factor_adj_factor is not None else None,
            'factor_open_hfq': float(self.factor_open_hfq) if self.factor_open_hfq is not None else None,
            'factor_open_qfq': float(self.factor_open_qfq) if self.factor_open_qfq is not None else None,
            'factor_close_hfq': float(self.factor_close_hfq) if self.factor_close_hfq is not None else None,
            'factor_close_qfq': float(self.factor_close_qfq) if self.factor_close_qfq is not None else None,
            'factor_high_hfq': float(self.factor_high_hfq) if self.factor_high_hfq is not None else None,
            'factor_high_qfq': float(self.factor_high_qfq) if self.factor_high_qfq is not None else None,
            'factor_low_hfq': float(self.factor_low_hfq) if self.factor_low_hfq is not None else None,
            'factor_low_qfq': float(self.factor_low_qfq) if self.factor_low_qfq is not None else None,
            'factor_pre_close_hfq': float(self.factor_pre_close_hfq) if self.factor_pre_close_hfq is not None else None,
            'factor_pre_close_qfq': float(self.factor_pre_close_qfq) if self.factor_pre_close_qfq is not None else None,
            'factor_macd_dif': float(self.factor_macd_dif) if self.factor_macd_dif is not None else None,
            'factor_macd_dea': float(self.factor_macd_dea) if self.factor_macd_dea is not None else None,
            'factor_macd': float(self.factor_macd) if self.factor_macd is not None else None,
            'factor_kdj_k': float(self.factor_kdj_k) if self.factor_kdj_k is not None else None,
            'factor_kdj_d': float(self.factor_kdj_d) if self.factor_kdj_d is not None else None,
            'factor_kdj_j': float(self.factor_kdj_j) if self.factor_kdj_j is not None else None,
            'factor_rsi_6': float(self.factor_rsi_6) if self.factor_rsi_6 is not None else None,
            'factor_rsi_12': float(self.factor_rsi_12) if self.factor_rsi_12 is not None else None,
            'factor_rsi_24': float(self.factor_rsi_24) if self.factor_rsi_24 is not None else None,
            'factor_boll_upper': float(self.factor_boll_upper) if self.factor_boll_upper is not None else None,
            'factor_boll_mid': float(self.factor_boll_mid) if self.factor_boll_mid is not None else None,
            'factor_boll_lower': float(self.factor_boll_lower) if self.factor_boll_lower is not None else None,
            'factor_cci': float(self.factor_cci) if self.factor_cci is not None else None,

            # 资金流向
            'moneyflow_pct_change': float(self.moneyflow_pct_change) if self.moneyflow_pct_change is not None else None,
            'moneyflow_latest': float(self.moneyflow_latest) if self.moneyflow_latest is not None else None,
            'moneyflow_net_amount': float(self.moneyflow_net_amount) if self.moneyflow_net_amount is not None else None,
            'moneyflow_net_d5_amount': float(self.moneyflow_net_d5_amount) if self.moneyflow_net_d5_amount is not None else None,
            'moneyflow_buy_lg_amount': float(self.moneyflow_buy_lg_amount) if self.moneyflow_buy_lg_amount is not None else None,
            'moneyflow_buy_lg_amount_rate': float(self.moneyflow_buy_lg_amount_rate) if self.moneyflow_buy_lg_amount_rate is not None else None,
            'moneyflow_buy_md_amount': float(self.moneyflow_buy_md_amount) if self.moneyflow_buy_md_amount is not None else None,
            'moneyflow_buy_md_amount_rate': float(self.moneyflow_buy_md_amount_rate) if self.moneyflow_buy_md_amount_rate is not None else None,
            'moneyflow_buy_sm_amount': float(self.moneyflow_buy_sm_amount) if self.moneyflow_buy_sm_amount is not None else None,
            'moneyflow_buy_sm_amount_rate': float(self.moneyflow_buy_sm_amount_rate) if self.moneyflow_buy_sm_amount_rate is not None else None,
            ...
            'ma30': float(self.ma30) if self.ma30 is not None else None,
            'ma60': float(self.ma60) if self.ma60 is not None else None,
            'ma120': float(self.ma120) if self.ma120 is not None else None,
        }
