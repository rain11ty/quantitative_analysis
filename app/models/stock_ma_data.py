...
class StockMaData(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            ...
            'ema5': float(self.ema5) if self.ema5 is not None else None,
            'ema10': float(self.ema10) if self.ema10 is not None else None,
            'ema20': float(self.ema20) if self.ema20 is not None else None,
            'ema30': float(self.ema30) if self.ema30 is not None else None,
            'ema60': float(self.ema60) if self.ema60 is not None else None,
            'ema120': float(self.ema120) if self.ema120 is not None else None
        }
